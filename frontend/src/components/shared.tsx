...
export function CampaignContext({ campaign }: { campaign: Campaign }) {
  return (
    <div className="context card">
      <div className="bank-avatar">{campaign.bank_name.slice(0, 2).toUpperCase()}</div>
      <div>
        <h3>{campaign.details?.campaign_name || campaign.bank_name}</h3>
        <p>
          {campaign.bank_name} <span className="dot">·</span> {label(campaign.project)}{" "}
  ...
  return (
    <nav className="steps" aria-label="Analytical workflow">
      {links.map((step, index) => (
        <div key={step.name} className={current === index + 1 ? "active" : ""}>
