...
function LabelEditor({ id, review }: { id: string; review: boolean }) {
  ...
  useEffect(() => {
    let active = true;
    setLoadError("");
    Promise.all([getFeatures(id), getSuggestions(id)]).then(([result, suggestion]) => {
      if (!active) return;
      ...
      if (review) {
        if (localStorage.getItem(storageKey)) throw new Error("You have an unsaved manual draft. Open manual labeling and save it before reviewing suggestions.");
        if (!suggestion || suggestion.reviewed) throw new Error("No pending suggestions. Open manual labeling or generate suggestions again.");
        for (const field of featureFields) {
          if (loaded[field.key] === null && suggestion.values[field.key] !== undefined) {
  ...
  function change(key: FeatureKey | "labeling_notes", value: string | number | boolean | null) {
    ...
    try { if (!reviewPending) localStorage.setItem(storageKey, JSON.stringify(next)); }
  ...
  const save = useCallback(async (complete = false) => {
    ...
    try {
      ...
      if (reviewPending && proposal) {
        ...
        setResponse(result); setDirty(false); setReviewPending(false);
  ...
  if (loadError) return <><Notice message={loadError} /><button onClick={() => setRetry(v => v + 1)}>Try again</button> <Link to={`/campaigns/${id}/label`}>Manual labeling</Link> · <Link to="/">All campaigns</Link></>;
  ...
  return <>
    ...
    <div className="page-heading"><div><h1>Campaign Feature Framework</h1><p>{reviewPending ? "Review automatic suggestions" : "Manual Labeling"}</p></div><FeatureStatus value={dirty ? "In Progress" : response.labeling_status} /></div>
    ...
    {campaign.collection?.is_demo && <div className="notice" role="note">Demo imported content — not collected website evidence.</div>}
    {proposal && !proposal.reviewed && <section className="notice" aria-label="Automatic suggestions">
      <strong>{proposal.is_demo ? "Demo automatic suggestions" : "Automatic suggestions"}</strong>
      {reviewPending ? <><p>Review each value before saving. Existing manual values are kept; empty fields are prefilled. Autosave is off during review. Unsaved review edits reset on reload.</p>
        <details><summary>Review proposed values</summary><ul>{featureFields.filter(f => proposal.values[f.key] !== undefined).map(field => <li key={field.key}>{label(field.key)}: {String(proposal.values[field.key] ?? "Unset")} <button type="button" className="secondary" disabled={saving} onClick={() => change(field.key, proposal.values[field.key] ?? null)}>Use suggestion</button></li>)}</ul></details>
        {proposal.warnings.map(warning => <p key={warning}>{warning}</p>)}</> : <p><Link to={`/campaigns/${id}/label?review=1`}>Review pending suggestions</Link>. Manual editing does not apply them.</p>}
    </section>}
    ...
    <form className="label-editor" onSubmit={e => { e.preventDefault(); void save(); }}>
        ...
        {sections.map((section, i) => {
          ...
          return <section id="active-label-section" className="card feature-section" key={section} aria-labelledby="active-section-title">
            ...
            <div className="form-grid">{fields.map(field => {
              ...
              const suggested = reviewPending && proposal?.values[field.key] !== undefined;
              const proposedValue = proposal?.values[field.key];
              const proposedText = field.kind === "scale" && typeof proposedValue === "number" ? `${proposedValue} — ${field.descriptions[proposedValue - 1]}` : String(proposedValue ?? "Unset");
              const help = suggested ? `${field.help}. Auto suggested: ${proposedText}. You can keep, change, or clear this value.` : field.help;
              ...
              if (field.kind === "scale") return <RatingScale key={field.key} label={label(field.key)} help={help} value={typeof value === "number" ? value : null} descriptions={field.descriptions} onChange={v => change(field.key, v)} />;
              if (field.kind === "boolean") return <BooleanChoice key={field.key} label={label(field.key)} help={help} value={typeof value === "boolean" ? value : null} onChange={v => change(field.key, v)} />;
      ...
      <div className="card form-card labeling-actions"><span role="status">{saving ? "Saving…" : error ? "Save failed" : reviewPending ? "Suggestions awaiting review" : dirty ? "Changes pending…" : "Saved ✓"}</span><button type="submit" className="secondary" disabled={saving}>{reviewPending ? "Save Reviewed Draft" : "Save Draft"}</button><button type="button" className={activeSection === sections.length - 1 || progress === 100 ? "" : "secondary"} disabled={saving} onClick={() => void save(true)}>Complete Labeling</button><Link to={`/compare?product_category=${encodeURIComponent(campaign.project)}`}>Compare this category</Link><Link to="/">Dataset</Link></div>
    </form>
  </>;
}
