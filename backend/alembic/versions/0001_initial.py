...
def upgrade():
    ...
    op.create_table(
        ...
        *[
            ...
            for name, kind in [
                ("campaign_name", sa.String(300)),
                ("headline", sa.Text()),
                ("subheadline", sa.Text()),
                ("main_message", sa.Text()),
                ("cta_text", sa.String(300)),
                ("notes", sa.Text()),
                ("text_density", sa.String(40)),
                ("tone", sa.String(120)),
                ("feature_vs_benefit", sa.String(40)),
                ("emotional_vs_rational", sa.String(40)),
    ...
    op.create_table(
        ...
        *[
            sa.CheckConstraint(f"{name} BETWEEN 1 AND 5", name=f"ck_{name}")
            for name in scores
        ],
    )
