...
def upgrade():
    ...
    op.execute(
        "INSERT INTO banks (name, normalized_name) SELECT MIN(bank_name), LOWER(bank_name) FROM campaigns WHERE LOWER(bank_name) NOT IN (SELECT normalized_name FROM banks) GROUP BY LOWER(bank_name)"
    )
    op.execute(
        "INSERT INTO projects (key, name) SELECT DISTINCT project, project FROM campaigns WHERE project NOT IN (SELECT key FROM projects)"
    )
