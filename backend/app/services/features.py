...
def feature_response(db: Session, campaign_id: int):
    ...
    return {
        ...
        "progress": round(100 * (len(FEATURE_FIELDS) - len(missing)) / len(FEATURE_FIELDS)),
...
def save_features(db: Session, campaign_id: int, data: FeatureInput):
    ...
    record.source = "manual_override" if record.source in ("automatic", "manual_override") else "manual"
...
def complete_features(db: Session, campaign_id: int, confirm_incomplete: bool):
    ...
    if record is None or len(missing_fields(record)) == len(FEATURE_FIELDS):
        raise HTTPException(status_code=422, detail="Label at least one feature before completing.")
    FeatureRead.model_validate(record)
    if missing_fields(record) and not confirm_incomplete:
        raise HTTPException(status_code=422, detail="Some features are unset. Confirm incomplete labeling to complete this record.")
