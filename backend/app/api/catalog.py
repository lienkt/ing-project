...
def persist(db: Session, record):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=409, detail="This name already exists. Choose a different name."
        )
    db.refresh(record)
    return record
...
def add_project(data: CatalogInput, db: DB):
    ...
    if not key or len(key) > 40:
        raise HTTPException(
            status_code=422,
            detail="Use a project name that produces 1–40 English letters, digits, or underscores.",
        )
...
def edit_bank(bank_id: int, data: CatalogInput, db: DB):
    ...
    with db.no_autoflush:
        db.execute(
            update(Campaign)
            .where(func.lower(Campaign.bank_name) == old_name)
            .values(bank_name=name, updated_at=datetime.now(timezone.utc))
        )
    bank.name = name
...
def delete_bank(bank_id: int, db: DB):
    ...
    if (
        ...
        raise HTTPException(
            status_code=409,
            detail="This bank is used by campaigns. Reassign or delete those campaigns first.",
        )
    db.delete(bank)
    db.commit()
...
def edit_project(project_key: str, data: CatalogInput, db: DB):
    ...
    with db.no_autoflush:
        db.execute(
            update(Campaign)
            .where(Campaign.project == project_key)
            .values(project=key, updated_at=datetime.now(timezone.utc))
        )
    project.key = key
    project.name = name
...
def delete_project(project_key: str, db: DB):
    project = find_option(db, ProjectOption, project_key)
    if (
        db.scalar(select(Campaign.id).where(Campaign.project == project_key).limit(1))
        is not None
    ):
        raise HTTPException(
            status_code=409,
            detail="This project is used by campaigns. Reassign or delete those campaigns first.",
        )
    db.delete(project)
    db.commit()
    return Response(status_code=204)
