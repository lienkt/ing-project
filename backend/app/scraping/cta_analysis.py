...
from app.scraping import cta_analysis_config as config
...
def _looks_like_cta(text: str, tag: str, role: str, classes: str) -> bool:
    ...
    semantic_button = (
        tag.casefold() in {"button", "input"} or role.casefold() == "button"
    )
    return (
        keyword_match or _has_cta_class(classes) or (semantic_button and len(text) >= 2)
    )
...
def _is_excluded(text: str) -> bool:
    ...
    return any(
        normalize_text(excluded) in normalized_text for excluded in config.EXCLUDE_TEXT
    )
...
async def extract_cta_features(page: Page, url: str) -> dict[str, Any]:
    ...
    return await extract_loaded_cta_features(page)


async def extract_loaded_cta_features(page: Page) -> dict[str, Any]:
    """Analyze an already rendered page without navigating or changing evidence."""
    return summarize_candidates(
        await _collect_candidates(page),
        (page.viewport_size or config.VIEWPORT)["height"],
    )


def summarize_candidates(
    candidates: list[CtaCandidate], viewport_height: int
) -> dict[str, Any]:
    """Apply the original deduplication, ranking, and scoring rules."""
    ...
    text_counts = Counter(normalize_text(item["text"]) for item in unique_candidates)
    result = {
        ...
        "cta_prominence": _prominence_score(primary, viewport_height) if primary else 1,
...
def extract_loaded_cta_features_sync(page) -> dict[str, Any]:
    """Use the same scoring rules with the existing synchronous message renderer."""
    candidates = []
    elements = page.locator(config.CTA_SELECTOR)
    for index in range(elements.count()):
        element = elements.nth(index)
        try:
            if not element.is_visible() or element.evaluate(
                "e => Boolean(e.closest('header, nav, footer'))"
            ):
                continue
            text = clean_text(element.inner_text(timeout=1_200))
            if not text:
                text = clean_text(
            ...
            if not _looks_like_cta(
                text, tag, element.get_attribute("role") or "", classes
            ):
                continue
            box = element.bounding_box()
            if not box:
                continue
            style = element.evaluate("""e => {
            ...
            candidates.append(
                dict(
                    ...
                    classes=classes,
                    y=round(box["y"] + page.evaluate("window.scrollY"), 1),
                    area=round(box["width"] * box["height"], 1),
                    **style,
                )
            )
            ...
            continue
    return summarize_candidates(
        candidates, (page.viewport_size or config.VIEWPORT)["height"]
    )
