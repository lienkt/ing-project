...
def copy_columns(record, excluded):
    return {column.name: getattr(record, column.name)
            for column in record.__table__.columns if column.name not in excluded}
...
def move_existing(source_engine, demo_engine, demo_urls):
    ...
    with Session(source_engine) as source, source.begin():
        records = list(source.scalars(select(Campaign).where(
            Campaign.campaign_url.in_(demo_urls), Campaign.bank_name.startswith("DEMO — "))))
        if not records:
            return 0
        with Session(demo_engine) as destination, destination.begin():
            for original in records:
                if destination.scalar(select(Campaign.id).where(Campaign.campaign_url == original.campaign_url)):
                    raise RuntimeError("A demo URL already exists in the destination. No source records were removed; resolve the duplicate before moving.")
                ...
                if destination.get(ProjectOption, project) is None:
                    destination.add(ProjectOption(key=project, name=project.replace('_', ' ').title()))
                if destination.scalar(select(Bank.id).where(Bank.normalized_name == bank_name.lower())) is None:
                    destination.add(Bank(name=bank_name, normalized_name=bank_name.lower()))
                copied = Campaign(**{**copy_columns(original, {'id'}), 'bank_name': bank_name, 'project': project})
                if original.details:
                    values = copy_columns(original.details, {'campaign_id'})
                    if values['campaign_name']:
                        values['campaign_name'] = values['campaign_name'].removeprefix("DEMO — ")
                    copied.details = CampaignDetails(**values)
                if original.features:
                    values = copy_columns(original.features, {'campaign_id'})
                    if values['product_name']:
                        values['product_name'] = values['product_name'].removeprefix("DEMO — ")
                    copied.features = CampaignFeature(**values)
                if original.evaluation:
                    copied.evaluation = Evaluation(**copy_columns(original.evaluation, {'id', 'campaign_id'}))
        ...
        source.flush()
        for bank in source.scalars(select(Bank).where(Bank.normalized_name.in_(old_banks))):
            if source.scalar(select(Campaign.id).where(func.lower(Campaign.bank_name) == bank.normalized_name).limit(1)) is None:
                source.delete(bank)
        for project in old_projects:
            if project.startswith('demo_') and source.scalar(select(Campaign.id).where(Campaign.project == project).limit(1)) is None:
...
def main():
    ...
    parser.add_argument('--move-existing', action='store_true', help='Move legacy demo fixtures out of the real database, preserving edits.')
    ...
    if args.move_existing:
        ...
        moved = move_existing(real_engine, demo_engine, [str(row.campaign_url) for row in rows])
        real_engine.dispose()
        print(f'Moved {moved} legacy demo campaigns; existing research campaigns preserved.')
    with Session(demo_engine) as db, db.begin():
        added = seed_demo(db, rows)
    demo_engine.dispose()
    print(f'Added {added} demo campaigns. Set DATA_MODE=demo in backend/.env and restart the backend.')
