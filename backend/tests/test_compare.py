...
def add_page(client, bank, category="current_account"):
    response = client.post("/api/campaigns", json={
        "bank_name": bank, "project": category,
...
def test_comparison_multiple_pages_and_real_values(client):
    ...
    client.put(f"/api/campaigns/{first}/features", json={
        "word_count": 120, "paragraph_count": 8, "text_density": 2,
        "price_visible": False, "cta_count": 0, "bank_type": "Traditional",
        "product_name": "Account A", "language": "Dutch", "capture_date": "2026-09-15",
    })
    client.put(f"/api/campaigns/{second}/features", json={"text_density": 5})
    client.post(f"/api/campaigns/{second}/features/complete", json={"confirm_incomplete": True})
    response = client.get("/api/compare", params={"product_category": "current_account"})
...
def test_comparison_filters_category_ids_and_banks(client):
    ...
    params = [("product_category", "current_account"), ("campaign_ids", first),
              ("campaign_ids", second), ("campaign_ids", third),
              ("campaign_ids", other_category), ("bank_names", "ing")]
    ...
    assert client.get("/api/compare", params={"product_category": "current_account", "campaign_ids": other_category}).json()["pages"] == []


def test_comparison_missing_features_and_empty_selection(client):
    campaign_id = add_page(client, "KBC")
    page = client.get("/api/compare", params={"product_category": "current_account"}).json()["pages"][0]
    ...
    assert client.get("/api/compare", params={"product_category": "mortgage"}).json()["pages"] == []
    assert client.get("/api/compare", params={"product_category": "current_account", "campaign_ids": 999999}).json()["pages"] == []
    assert client.get("/api/compare").status_code == 422
    assert client.get("/api/compare", params={"product_category": ""}).status_code == 422
