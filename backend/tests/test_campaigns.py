...
SCORES = {
    ...
    "cta_score": 4,
    "overall_score": 2,
    "evaluation_notes": "Strong visuals",
}
...
def test_get_campaigns_and_filters(client):
    ...
    assert (
        len(
            client.get(
                "/api/campaigns", params={"bank_name": "kbc", "project": "credit_card"}
            ).json()
        )
        == 1
    )
...
def test_update_details(client):
    ...
    response = client.put(
        url, json={"headline": "Value for you", "text_density": "medium"}
    )
    assert response.status_code == 200
    assert response.json()["status"] == "Details Added"
    assert (
        client.put(url, json={"notes": "Updated"}).json()["details"]["headline"]
        == "Value for you"
    )
    response = client.put(
        url, json={"headline": None, "text_density": None, "notes": " "}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ("bank_name", "  "),
        ("project", "invalid"),
        ("campaign_url", "javascript:alert(1)"),
    ],
)
def test_invalid_campaign(client, field, value):
    assert (
        client.post("/api/campaigns", json={**BASIC, field: value}).status_code == 422
    )
...
def test_invalid_scores(client, score):
    ...
    assert (
        client.post(
            f"/api/campaigns/{campaign['id']}/evaluation",
            json={**SCORES, "clarity_score": score},
        ).status_code
        == 422
    )
...
def test_not_found_and_invalid_details(client):
    ...
    assert (
        client.put(
            f"/api/campaigns/{campaign['id']}/details", json={"text_density": "huge"}
        ).status_code
        == 422
    )
...
def test_edit_basic_after_evaluation_preserves_work(client):
    ...
    updated = client.put(
        ...
        json={
            "bank_name": "ING",
            "project": "mortgage",
            "campaign_url": "https://example.com/mortgage",
        },
    )
...
def test_catalog_options_and_custom_campaign(client):
    ...
    assert (
        client.post("/api/projects", json={"name": "business loan"}).status_code == 409
    )
    response = client.post(
        "/api/campaigns",
        json={**BASIC, "bank_name": "Argenta", "project": "business_loan"},
    )
    assert response.status_code == 201
    assert (
        client.get(f"/api/campaigns/{response.json()['id']}").json()["project"]
        == "business_loan"
    )
    assert (
        len(client.get("/api/campaigns", params={"project": "business_loan"}).json())
        == 1
    )
...
def test_bank_edit_delete_and_linked_campaigns(client):
    ...
    assert (
        client.get(f"/api/campaigns/{campaign['id']}").json()["bank_name"] == "KBC Bank"
    )
    assert client.delete(url).status_code == 409
...
def test_project_edit_delete_and_linked_campaigns(client):
    ...
    assert (
        client.put("/api/projects/credit_card", json={"name": "Mortgage"}).status_code
        == 409
    )
    assert client.get(campaign_url).json()["project"] == "credit_card"
    assert (
        client.put(
            "/api/projects/credit_card", json={"name": "Premium Card"}
        ).status_code
        == 200
    )
    ...
    assert (
        client.put(campaign_url, json={**BASIC, "project": "mortgage"}).status_code
        == 200
    )
    ...
    assert (
        client.put("/api/projects/premium_card", json={"name": "Missing"}).status_code
        == 404
    )
