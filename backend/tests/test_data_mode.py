...
def test_database_selection_and_separation():
    ...
    with pytest.raises(ValidationError):
        Settings(
            _env_file=None,
            demo_database_url=real.database_url.replace("localhost", "127.0.0.1"),
        )
...
def test_move_demo_preserves_research_and_label_edits():
    ...
    with Session(source) as db, db.begin():
        ...
        real = Campaign(
            ...
            campaign_url="https://example.com/research",
        )
        demo = Campaign(
            bank_name="DEMO — ING", project="demo_current_account", campaign_url=url
        )
        demo.details = CampaignDetails(campaign_name="DEMO — Changed title")
        demo.features = CampaignFeature(
            word_count=1234,
            labeling_status="Completed",
            product_name="DEMO — Changed product",
        )
...
def test_demo_seed_repeat_preserves_edits():
    ...
    rows = TypeAdapter(list[DemoCampaign]).validate_python(
        json.loads(Path("data/demo_campaigns.json").read_text())
    )
    with Session(engine) as db, db.begin():
        ...
        assert all(
            not page.bank_name.startswith("DEMO")
            for page in db.scalars(select(Campaign))
        )
...
def test_auto_demo_examples_preserve_pending_review_and_existing_edits():
    ...
    with Session(engine) as db, db.begin():
        ...
        assert (
            db.scalar(
                select(Campaign).where(Campaign.bank_name == "KBC")
            ).features.word_count
            == 12345
        )
    engine.dispose()
