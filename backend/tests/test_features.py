...
def test_feature_lifecycle(client):
    ...
    saved = client.put(path, json={'word_count': 0, 'has_hero_image': False, 'text_density': 3}).json()
...
SCALE_FIELDS = [c.name.removeprefix('ck_cf_') for c in CampaignFeature.__table__.constraints if c.name and 'BETWEEN 1 AND 5' in str(c.sqltext)]
COUNT_FIELDS = [c.name.removeprefix('ck_cf_') for c in CampaignFeature.__table__.constraints if c.name and '>= 0' in str(c.sqltext)]
...
def test_invalid_scales(client, field, value):
    ...
    assert client.put(f'/api/campaigns/{campaign_id}/features', json={field: value}).status_code == 422

@pytest.mark.parametrize('field', COUNT_FIELDS)
def test_negative_counts(client, field):
    campaign_id = create(client)['id']
    assert client.put(f'/api/campaigns/{campaign_id}/features', json={field: -1}).status_code == 422


def test_empty_completion_and_invalid_choices(client):
    campaign_id = create(client)['id']
    path = f'/api/campaigns/{campaign_id}/features'
    assert client.post(path + '/complete', json={'confirm_incomplete': True}).status_code == 422
...
def test_extended_metadata_and_derived_average(client):
    ...
    path = f"/api/campaigns/{campaign['id']}/features"
    payload = {
        "bank_type": "Traditional", "product_name": "ING Lion Account",
        "language": "Dutch", "capture_date": "2026-09-15",
        "headline_length": 6, "word_count": 100, "paragraph_count": 8,
    }
    ...
    assert client.put(path, json={'word_count': 0}).json()['features']['average_paragraph_length'] == 0
    assert client.put(path, json={'paragraph_count': 0}).json()['features']['average_paragraph_length'] is None
    assert client.put(path, json={'word_count': None, 'paragraph_count': 4}).json()['features']['average_paragraph_length'] is None
    assert client.put(path, json={'headline_length': None}).json()['features']['headline_length'] is None
    for invalid in ({'bank_type': 'Unknown'}, {'language': 'German'}, {'capture_date': '2026-02-30'}, {'headline_length': -1}, {'average_paragraph_length': 50}):
